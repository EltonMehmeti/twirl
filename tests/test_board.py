...
def test_today_board_groups_bookings(db):
    ...
    pending = make_booking(
        db,
        items[0],
        pickup=T + timedelta(days=9),
        return_=T + timedelta(days=10),
        kind="online",
        status="pending_shop",
    )
    pickup = make_booking(db, items[1], pickup=T, return_=T + timedelta(days=2))
    returning = make_booking(
        db, items[2], pickup=T - timedelta(days=2), return_=T, status="picked_up"
    )
    overdue = make_booking(
        db, items[3], pickup=T - timedelta(days=1), return_=T + timedelta(days=1)
    )
...
def test_week_calendar_places_bookings_including_cleaning_day(db):
    ...
    booking = make_booking(
        db, item, pickup=T + timedelta(days=1), return_=T + timedelta(days=2), cleaning_days=1
    )
    days, rows = week_calendar(db, shop.id, T)
    assert len(days) == 7 and days[0] == T
    cells = rows[0].cells
    assert [c.id if c else None for c in cells[:5]] == [
        None,
        booking.id,
        booking.id,
        booking.id,
