...
ACTIONS = {
    ...
    "confirmed": [
        ("picked-up", N_("Picked up"), False),
        ("no-show", N_("No-show"), False),
        ("cancel", N_("Cancel"), True),
    ],
    "at_risk": [("cancel", N_("Cancel"), True)],
    "picked_up": [
        ("returned-ok", N_("Returned OK"), False),
...
def _booking_page(
    ...
    *,
    ...
    candidates = (
        ...
        if not is_block and booking.status in staff.SWAPPABLE
        else []
    )
    return render(
        request,
...
def today(
    request: Request, ctx: ShopContext = Depends(require_shop), db: Session = Depends(get_db)
):
    day = clock.today()
    return render(
        request,
...
def _walk_in_page(
    ...
    *,
    ...
    item = (
        staff.find_item_by_code(db, ctx.shop.id, values["code"])
        if len(values["code"]) == 4
        else None
    )
    return render(
        request,
...
def walk_in_form(
    ...
    values = {
        ...
        "return_date": (day + timedelta(days=2)).isoformat(),
        "name": "",
        "phone": "",
        "note": "",
        "picked_up_now": True,
...
def create_walk_in(
    ...
    if parsed is None:
        return _walk_in_page(
            request,
            db,
            ctx,
            values,
            errors=errors,
            error=N_("Check the highlighted fields."),
            status_code=400,
        )
    ...
    if item is None:
        return _walk_in_page(
            ...
            ctx,
            values,
            error=N_("No dress with this code in your shop."),
            status_code=400,
        )
    try:
        booking = staff.create_staff_booking(
            ...
            shop=ctx.shop,
            item=item,
            pickup=parsed.pickup_date,
            return_=parsed.return_date,
            name=parsed.name,
            phone=parsed.phone,
            actor=ctx.actor,
            kind=BookingKind(parsed.kind),
            picked_up_now=parsed.picked_up_now,
            override_reason=parsed.override_reason or None,
            note=parsed.note,
        )
        ...
        return _walk_in_page(
            ...
            error=N_("This dress is already booked on those dates."),
            status_code=409,
        )
    except InvalidPhone:
        db.rollback()
        return _walk_in_page(
            ...
            ctx,
            values,
            error=N_("That phone number does not look right."),
            status_code=400,
        )
        ...
        return _walk_in_page(
            ...
            ctx,
            values,
            error=DATE_ERRORS.get(exc.code, GENERIC_DATE_ERROR),
            status_code=400,
        )
    db.commit()
    return RedirectResponse(f"/shop/bookings/{booking.id}", status_code=303)
